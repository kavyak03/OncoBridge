...
This script overwrites only generated mock assets under data/:
...
def main():
    ...
    safe_unlink(sqlite_path, allowed_roots=[DATA])
    ...
    cur.executemany(
        ...
        list(zip(sample_ids, patient_ids, diagnosis, ages.tolist(), dates, strict=True)),
    )
    ...
    expr_uri = TDB / "expression_array"
    var_uri = TDB / "variants_array"
    for uri in [expr_uri, var_uri]:
        safe_rmtree(uri, allowed_roots=[DATA])
    ...
    dom = tiledb.Domain(
        tiledb.Dim(
            name="sample_id", domain=(min(sample_ids), max(sample_ids)), tile=10, dtype="ascii"
        ),
    ...
    tiledb.SparseArray.create(str(expr_uri), schema)
    ...
    with tiledb.SparseArray(str(expr_uri), mode="w") as array:
        array[df["sample_id"].tolist(), df["gene"].tolist()] = {
            "expression_value": df["expression_value"].to_numpy()
        }
    ...
    tiledb.SparseArray.create(str(var_uri), schema2)
    ...
    with tiledb.SparseArray(str(var_uri), mode="w") as array:
        array[df2["sample_id"].tolist(), df2["var_id"].tolist()] = {
