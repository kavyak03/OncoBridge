...
from ehr_fhir_genomics_toolkit.survival import (
    add_demo_survival_outcomes,
    fit_cox_example,
...
def parse_args():
    ...
    p.add_argument(
        "--add-demo-outcomes",
        action="store_true",
        help="Add synthetic survival_time_months/event_observed if not present.",
    )
    return p.parse_args()
...
def main():
    ...
    if {"survival_time_months", "event_observed"} - set(df.columns):
        if args.add_demo_outcomes:
            ...
            raise SystemExit(
                "Input CSV missing survival_time_months/event_observed. Use --add-demo-outcomes for an educational demo."
            )
