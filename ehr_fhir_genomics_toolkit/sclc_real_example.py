...
def convert_clinical(annotation_path: str | Path) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "diagnosis": ann.get("disease", "SCLC").astype(str).replace({"SCLC": "small cell lung cancer"}),
        "age_at_collection": pd.to_numeric(ann.get("age"), errors="coerce").fillna(-1).astype(int),
        ...
        "event_observed": ann.get("survivalStatus").map(lambda x: int(bool(x)) if pd.notna(x) else None),
...
def convert_coarse_therapy(annotation_path: str | Path) -> pd.DataFrame:
    ann = pd.read_csv(annotation_path, sep="\t", low_memory=False).copy()
    start_date = _parse_date_series(ann.get("dateFirstDiagnosis")).fillna(_parse_date_series(ann.get("sampleDate"))).fillna("2020-01-01")
...
def convert_variants(variant_path: str | Path, annotation_path: str | Path) -> pd.DataFrame:
    ...
    keep_cols = [c for c in ["ID", "Chr", "Start", "Ref", "Alt", "Gene.refGene", "CADD_phred"] + sample_cols if c in var.columns]
    v = var[keep_cols].copy()
    long_df = v.melt(
        id_vars=[c for c in ["ID", "Chr", "Start", "Ref", "Alt", "Gene.refGene", "CADD_phred"] if c in v.columns],
    ...
    out = pd.DataFrame({
        ...
        "GENE": long_df.get("Gene.refGene", pd.Series(["unknown"] * len(long_df))).astype(str).str.split("[,;]").str[0],
        "GT": 1,
        "QUAL": pd.to_numeric(long_df.get("CADD_phred"), errors="coerce").fillna(0).astype(float),
...
def convert_expression_panel(expression_path: str | Path, annotation_path: str | Path, genes: Iterable[str] | None = None) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "expression_value": pd.to_numeric(long_df["expression_value"], errors="coerce").fillna(0).astype(float),
        "gene_id": long_df.get("EnsID"),
        "biotype": long_df.get("biotype"),
    })
    return out

def write_converted_bundle(annotation_path: str | Path, variant_path: str | Path, expression_path: str | Path, out_dir: str | Path, genes: Iterable[str] | None = None) -> dict:
    ...
    expr_panel.to_csv(expr_path, index=False)
    return {
